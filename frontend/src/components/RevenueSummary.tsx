...
interface RevenueData {
    property_id: string;
    total_revenue: string; // exact decimal string from the API — never a lossy float
    currency: string;
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', showRaw }) => {
    ...
    // Money is an exact decimal string (e.g. "2250.000"). Group the integer part for
    // readability without ever converting to a float — the 3rd decimal must survive.
    const [intPart, fracPart = ''] = data.total_revenue.split('.');
    const displayTotal = intPart.replace(/\B(?=(\d{3})+(?!\d))/g, ',') + (fracPart ? `.${fracPart}` : '');
